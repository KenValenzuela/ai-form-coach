...
const SPEED_PRESETS = {
  quick: {
    label: "Quick",
    frameStride: 4,
    analysisDownscale: 0.4,
    description: "Fastest first pass",
  },
  balanced: {
    label: "Balanced",
    frameStride: 3,
    analysisDownscale: 0.5,
    description: "Recommended demo mode",
  },
  detailed: {
    label: "Detailed",
    frameStride: 1,
    analysisDownscale: 0.75,
    description: "Slower, higher fidelity",
  },
} as const;
...
function getFileValidationError(file: File): string | null {
  ...
  if (sizeMb > MAX_FILE_SIZE_MB) {
    return `Video is ${sizeMb.toFixed(1)} MB. Max is ${MAX_FILE_SIZE_MB} MB.`;
  }
...
function UploadPhase({
  ...
  selectedStartFrameIndex,
  setSelectedStartTimeSec,
  setSelectedStartFrameIndex,
  frameStride,
  setFrameStride,
  analysisDownscale,
  setAnalysisDownscale,
  speedPreset,
  setSpeedPreset,
  onDrop,
  handleFile,
      ...
      >
        ...
        {file ? (
          <>
            ...
            <div style={{ fontWeight: 600, color: "var(--green)", maxWidth: 280, wordBreak: "break-word" }}>
              {file.name}
            </div>
            ...
            <div style={{ fontSize: 12, color: "var(--muted)" }}>
              First usable frame: {previewFrameNumber}. Scrub to a stable frame where the sleeve/end cap is visible,
              then set one bounding box.
            </div>
            ...
            {markerBox && (
              <div style={{ fontSize: 12, color: "var(--green)" }}>
                ROI confirmed ✓ (center {(markerBox.x + markerBox.w / 2).toFixed(3)},{" "}
                {(markerBox.y + markerBox.h / 2).toFixed(3)})
              </div>
            )}
          ...
          <>
            ...
            <div style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>
              MP4, MOV, AVI, WebM up to {MAX_FILE_SIZE_MB} MB
              <br />
      ...
      >
            ...
            {(Object.entries(SPEED_PRESETS) as [SpeedPreset, typeof SPEED_PRESETS[SpeedPreset]][]).map(
              ([key, preset]) => (
                ...
                >
                  ...
                  <span style={{ display: "block", fontSize: 11, fontWeight: 500, opacity: 0.78 }}>
                    {preset.description}
                  </span>
                </button>
              )
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <div>
            ...
            <select
              value={analysisDownscale}
              onChange={(e) => {
                setAnalysisDownscale(Number(e.target.value));
                setSpeedPreset("custom");
              }}
            >
        ...
        <div>
          ...
          >
            {canAnalyze && roiMode === "roiConfirmed"
              ? "Analyze with Tracking"
              : "Upload video, set + confirm one ROI, and confirm consent"}
          </button>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
              ...
              <button className="btn-ghost" type="button" onClick={() => setZoomPreview(false)}>
                Close
              </button>
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 2 }}>
              {exercise} · {apiResult?.rep_count ?? 0} rep{(apiResult?.rep_count ?? 0) !== 1 ? "s" : ""} ·{" "}
      ...
      : [
          {
            ...
            cue: "Film from different angles occasionally to get a full picture.",
            drill: null,
          },
        ];
          ...
          return (
            <button
              ...
              style={{
                width: "100%",
                textAlign: "left",
                background: activeId === i ? "var(--lav-d)" : "none",
                border: "none",
                borderRadius: 8,
                padding: "10px 12px",
                cursor: "pointer",
        ...
        {metrics.map((m) => {
          const tagClass =
            ({
              ...
              good: "tag-green",
              tip: "tag-lav",
            } as Record<string, string>)[m.sev] ?? "tag-lav";
      ...
      {apiResult?.warnings && apiResult.warnings.length > 0 && (
        <div className="card" style={{ padding: "16px 24px", borderColor: "oklch(88% .07 70)" }}>
          ...
          {apiResult.warnings.map((warn) => (
            <div key={warn} style={{ fontSize: 13, color: "var(--muted)", marginTop: 6 }}>
              ⚠️ {warn}
            </div>
          ))}
        </div>
...
function IssuesTab({ allIssues }: { allIssues: BackendIssue[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {goods.length > 0 && (
        <div className="card" style={{ padding: "20px 24px" }}>
          <div className="label" style={{ marginBottom: 12, color: "var(--green)" }}>
            What&apos;s Looking Good ({goods.length})
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          >
            ...
            <div style={{ position: "relative", width: "100%", maxHeight: 520, aspectRatio: "16/9" }}>
              ...
              >
                FPS:{" "}
                {apiResult?.tracking_summary?.average_fps
                  ? Math.round(apiResult.tracking_summary.average_fps)
                  : "--"}
              </div>
            ...
            >
              ...
              >
                <div>
                  ...
                  {(apiResult.tracking_summary.tracking_method_used ?? apiResult.tracking_summary.tracker_type).toUpperCase()}
                </div>

                <div>
                  <strong>Average FPS:</strong> {apiResult.tracking_summary.average_fps.toFixed(1)}
                </div>

                <div>
                  <strong>Success Rate:</strong>{" "}
                  {(apiResult.tracking_summary.tracking_success_rate * 100).toFixed(1)}%
                </div>

                <div>
                  <strong>Lost Frames:</strong> {apiResult.tracking_summary.lost_frames.length}
                </div>

                <div>
                  <strong>Path Points:</strong> {pathSource.length}
                </div>
              </div>

              {apiResult.tracking_summary.lost_frames.length > 0 && (
                <div style={{ marginTop: 8, color: "var(--amber)", fontSize: 12 }}>
                  ⚠️ Tracking lost on some frames; overlay marks degraded periods.
                </div>
              )}
            </div>
          )}
